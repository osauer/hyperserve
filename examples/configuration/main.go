...
func runProgrammaticConfig() {
	srv, err := serverpkg.NewServer(
		serverpkg.WithAddr(":8081"), // Different port
	...
	setupHandlers(srv, "Programmatic")

	fmt.Println("Configuration:")
	fmt.Printf("  Port: %s\n", srv.Options.Addr)
	fmt.Printf("  Rate Limit: %.0f req/s\n", float64(srv.Options.RateLimit))
...
func runEnvConfig() {
	...
	setupHandlers(srv, "Environment Variables")
...
func runCombinedConfig() {
	...
	srv, err := serverpkg.NewServer(
		serverpkg.WithAddr(":8086"),     // Will be overridden
		serverpkg.WithRateLimit(10, 20), // Will be overridden
	)
	if err != nil {
		log.Fatalf("Failed to create server: %v", err)
	}

	setupHandlers(srv, "Precedence Demo")
...
func setupHandlers(srv *serverpkg.Server, configType string) {
	srv.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		response := map[string]any{
			"message":     fmt.Sprintf("Server configured via: %s", configType),
			"port":        srv.Options.Addr,
			"rate_limit":  srv.Options.RateLimit,
			"burst_limit": srv.Options.Burst,
		}
