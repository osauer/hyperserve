...
func TestMCPHandler_ConcurrentRequests(t *testing.T) {
	...
	srv, err := hyperserve.New(
		...
		hyperserve.WithMCPSupport("concurrency-test-server", "1.0.0"),
		hyperserve.WithMCPBuiltinTools(true),     // Enable built-in tools for tests
		hyperserve.WithMCPBuiltinResources(true), // Enable built-in resources for tests
		hyperserve.WithMCPFileToolRoot(tempDir),
	)
	if err != nil {
...
func TestMCPResources_ThreadSafety(t *testing.T) {
	srv, err := hyperserve.New(
		hyperserve.WithAddr(":0"),
		hyperserve.WithMCPSupport("thread-safety-test", "1.0.0"),
		hyperserve.WithMCPBuiltinTools(true),     // Enable built-in tools for tests
		hyperserve.WithMCPBuiltinResources(true), // Enable built-in resources for tests
	)
	if err != nil {
