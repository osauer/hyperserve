...
**One runtime dependency.** `golang.org/x/time` for the rate limiter, and that's it. Vendoring is a five-second operation. The `go.sum` is short enough to actually read. Developer tooling, including the modernize analyzer, lives in a separate `tools/go.mod` graph and does not ship in your binary. If you have ever had to audit a transitive dependency tree for a security review, this matters. If you have not, it is a curiosity.
