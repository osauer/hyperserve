...
func TestWithTemplateDirInvalidDirectory(t *testing.T) {
	...
	_, err := New(WithTemplateDir("/non/existent/directory"))
	if err == nil {
...
func TestServerHeaderRejectsControlBytes(t *testing.T) {
	if _, err := New(WithServerHeader("example\r\nInjected: true")); err == nil {
...
func TestStartupBannerIsOptIn(t *testing.T) {
	...
	optedIn, err := New(WithStartupBanner())
	if err != nil {
		t.Fatalf("New with banner: %v", err)
	}
...
func TestWithCSPWebWorkerSupport(t *testing.T) {
	...
	srv, err = New(WithCSPWebWorkerSupport())
	if err != nil {
