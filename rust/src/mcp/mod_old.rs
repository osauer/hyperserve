...
fn handle_get_request(request: &Request, server_info: &MCPServerInfo, capabilities: &MCPCapabilities) -> Response {
    ...
    if is_json_accepted(accept) {
        ...
        let status = crate::json!({
            ...
            "capabilities": {
                "tools": {
                    "listChanged": capabilities.tools.as_ref().map(|t| t.list_changed).unwrap_or(false)
                },
                "resources": {
                    "subscribe": capabilities.resources.as_ref().map(|r| r.subscribe).unwrap_or(false),
                    "listChanged": capabilities.resources.as_ref().map(|r| r.list_changed).unwrap_or(false)
                },
                "sse": {
                    "enabled": capabilities.sse.as_ref().map(|s| s.enabled).unwrap_or(false),
                    "endpoint": capabilities.sse.as_ref().map(|s| s.endpoint.clone()).unwrap_or_else(|| "".to_string()),
                    "headerRouting": capabilities.sse.as_ref().map(|s| s.header_routing).unwrap_or(false)
                }
            },
...
fn handle_initialize(
    ...
    let _client_info = if let Some(params) = params {
        if let Some(client_obj) = params.get("clientInfo").and_then(|v| v.as_object()) {
            if let (Some(name), Some(version)) = (
                client_obj.get("name").and_then(|v| v.as_ref()),
                client_obj.get("version").and_then(|v| v.as_ref())
            ) {
                Some(MCPClientInfo {
                    name: name.to_string(),
    ...
    Ok(crate::json!({
        ...
        "instructions": "Follow the initialization protocol: after receiving this response, send an 'initialized' notification"
    }))
}
...
fn handle_tools_call(
    ...
    let tool_name = params.get("name").and_then(|v| v.as_ref()).ok_or_else(|| JsonRpcError {
        code: -32602,
...
fn handle_resources_list(resources: &Arc<Mutex<HashMap<String, Box<dyn MCPResource>>>>) -> Result<Value, JsonRpcError> {
    ...
    Ok(crate::json!({
        "resources": Value::Array(resource_list)
    }))
}
...
fn handle_resources_read(
    ...
    let uri = params.get("uri").and_then(|v| v.as_ref()).ok_or_else(|| JsonRpcError {
        code: -32602,
