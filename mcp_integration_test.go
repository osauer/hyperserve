...
func TestMCPOptimizationsIntegration(t *testing.T) {
	...
	t.Run("resource_caching", func(t *testing.T) {
		...
		var textData1, textData2 map[string]interface{}
		json.Unmarshal([]byte(contents1["text"].(string)), &textData1)
		json.Unmarshal([]byte(contents2["text"].(string)), &textData2)
		
		text1 := textData1["count"].(float64)
		text2 := textData2["count"].(float64)
