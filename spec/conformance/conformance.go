...
func testMCPProtocol(port int, result *testResult) {
	...
	// Test list resources
	resourcesRequest := map[string]any{
		"jsonrpc": "2.0",
...
func testEndpoint(port int, path string, method string, body any, expectedStatus int) bool {
	status := testEndpointStatus(port, path, method, body)
	return status == expectedStatus
}

func testEndpointStatus(port int, path string, method string, body any) int {
