...
New starts from deterministic defaults. Configuration sources and
...
The retired server-owned rate-limit settings rate_limit, burst,
HS_RATE_LIMIT, and HS_BURST_LIMIT are rejected when their source is bound.
Create middleware with ratelimit.New and attach it with Server.Use or
Server.UsePrefix instead.
...
func loadConfigFile(config *Options, path string) error {
	...
	retired := presentKeys(rawFields, "rate_limit", "burst")
	if len(retired) != 0 {
		return retiredRateLimitError("server configuration", retired)
	}
	fileConfig := &Options{}
...
func presentKeys(rawFields map[string]json.RawMessage, keys ...string) []string {
...
func retiredRateLimitError(source string, keys []string) error {
	quoted := make([]string, len(keys))
	for i, key := range keys {
		quoted[i] = fmt.Sprintf("%q", key)
	}
	return fmt.Errorf("%s contains retired server rate-limit setting(s) %s; use ratelimit.New to create middleware, then attach it with Server.Use or Server.UsePrefix", source, strings.Join(quoted, ", "))
}
...
// options. Options passed later to [New] override this snapshot.
...
// file is required to exist and contain one valid JSON object. The retired
// rate_limit and burst fields return an error with migration guidance.
...
// application chooses to read a file. Present HS_RATE_LIMIT or HS_BURST_LIMIT
// variables return an error with migration guidance, including empty values.
func WithEnvironment() Option {
	return func(srv *Server) error {
		...
		if len(retired) != 0 {
			return retiredRateLimitError("environment", retired)
		}
...
// These are the public `With*` knobs callers pass to [New]. They live
...
// Invalid HTTP control bytes cause [New] to return an error.
